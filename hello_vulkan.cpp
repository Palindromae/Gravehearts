...
void HelloVulkan::setup(const VkInstance& instance, const VkDevice& device, const VkPhysicalDevice& physicalDevice, uint32_t queueFamily)
{
  ...
  raytracer.setup(device, physicalDevice, &m_alloc, queueFamily);
...
void HelloVulkan::createDescriptorSetLayout()
{
  ...
  m_descSetLayoutBind.addBinding(SceneBindings::eModels, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                 VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR
                                     | VK_SHADER_STAGE_ANY_HIT_BIT_KHR);
...
void HelloVulkan::onResize(int /*w*/, int /*h*/)
{
  ...
  raytracer.updateRtDescriptorSet(m_offscreenColor.descriptor.imageView);
}
...
//  raytracer.createBottomLevelAS(m_objModel);
 // raytracer.createTopLevelAS(m_instances);
  raytracer.Tlas_obj.setup();
  raytracer.BuildChunkBLAS();
  raytracer.BuildTLAS();
  raytracer.createRtDescriptorSet(m_offscreenColor.descriptor.imageView);
  raytracer.createRaytracePipeline(m_descSetLayout);
}
...
void HelloVulkan::raytrace(const VkCommandBuffer& cmdBuf, const glm::vec4& clearColor)
{
  ...
  this->raytracer.raytrace(cmdBuf, clearColor, m_descSet, m_size, m_pcRaster);
}
