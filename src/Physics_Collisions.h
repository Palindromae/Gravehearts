...
static bool CollisionPoint(const float timestep, const glm::vec3 delta_velocity, const glm::vec3& ObjA_contact, const glm::vec3& ObjA_velocity, const glm::vec3& ObjB_contact, glm::vec3& PointOfCollision) {
...
static glm::vec3 FindMinTranslationVector(glm::vec3 vector, glm::vec3 minimumVector, const float BorderProject, const float VelocityProject, const float CurrentSeperatingDistance, float& MinSeperatingDistance, const bool currentSeperation,  float& timeToTarget)
{
...
static bool SeperationTest(const float BorderProject, const float VelocityProject, const float CurrentSeperatingDistance)
{
...
static bool SATOptimised(const glm::vec3* PositionsBuffer, const glm::quat* RotationBuffer, const int objA, glm::vec3 ASize, int objB, glm::vec3 BSize, SATCollision& collisionInfo, glm::vec3 DeltaVelocity)
{
    ASize /= 2.0f;
    BSize /= 2.0f;
...
static void UpdateAngularVelocity(float deltaTime, uint32_t ID, glm::quat* AngularVelocity, glm::quat* RotationBuffer, glm::quat* RotationBuffer_Past) {
	AngularVelocity[ID] = RotationBuffer[ID] * glm::inverse(RotationBuffer_Past[ID]) / deltaTime;
}

static void UpdateVelocity(float deltaTime, uint32_t ID, glm::vec3* VelocityBuffer, glm::vec3* PositionBuffer, glm::vec3* PositionBuffer_Past) {
...
static glm::vec3 ClosingVelocity(glm::vec3 Velocity, glm::quat AngularVelocity, glm::vec3 rotationArm) {
	return Velocity + glm::cross(AngularVelocity, rotationArm);
}

static void ApplyForce(float deltaTime, glm::vec3 force, const uint32_t ID, glm::vec3* VelocityBuffer) {
	
	VelocityBuffer[ID] += force * deltaTime;
}

static bool ResolvePositionAfterCollision(const float deltaTime, glm::vec3& contact_point, const glm::vec3 ABDeltaPositionResolution, const glm::vec3 normal, const PhysicsComponent& objA, const uint32_t objA_id, const PhysicsComponent& objB, const uint32_t objB_id, glm::vec3* PositionBuffer, glm::vec3* VelocityBuffer) {
...
static void ResolvePositionAfterCollision_World(const PhysicsRayCollision collision, const PhysicsComponent& objA, const uint32_t objA_id, glm::vec3* PositionBuffer, glm::vec3* VelocityBuffer) {
    PositionBuffer[objA_id] += glm::normalize(VelocityBuffer[objA_id])* collision.distanceToPosition;
}


static void ResolveVelocitiesAfterCollision(const float deltaTime, float distanceToPosition, glm::vec3 contact_point, glm::vec3 normal,
	...
	AngularVelocityBuffer[objA_id] *= glm::quat(objA.InverseInertia * glm::cross(Velocity_Impulse, A_rotation_arm));
	AngularVelocityBuffer[objB_id] *= glm::quat(-objB.InverseInertia * glm::cross(Velocity_Impulse, B_rotation_arm));
	...
	AngularVelocityBuffer[objA_id] *= glm::quat(objA.InverseInertia * glm::cross(FrictionImpulse, A_rotation_arm));
	AngularVelocityBuffer[objB_id] *= glm::quat(-objB.InverseInertia * glm::cross(FrictionImpulse, B_rotation_arm));
	

}


static void ResolveVelocitiesAfterCollision_World(const float deltaTime, float distanceToPosition, glm::vec3 contact_point, glm::vec3 normal,
    glm::vec3* PositionBuffer, glm::quat* RotationBuffer, glm::vec3* VelocityBuffer, glm::quat* AngularVelocityBuffer,
    glm::vec3* PositionBuffer_Past, glm::quat* RotationBuffer_Past, glm::vec3* VelocityBuffer_Past, glm::quat* AngularVelocityBuffer_Past,
